...
class DFABackend(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if not dfa_context.no_training:
            random_projection = torch.mm(
                grad_output.reshape(grad_output.shape[0], -1).to(dfa_context.rp_device), dfa_context.feedback_matrix
            )  # Global random projection
            if dfa_context.normalization:
                random_projection /= torch.sqrt(torch.prod(random_projection.shape[1:]))
...
class DFA(nn.Module):
    ...
    def forward(self, input):
        if not (self.initialized or self.no_training):
            ...
            self.output_size = torch.prod(input.shape[1:])

            for layer in self.dfa_layers:
                feedback_size = torch.prod(layer.feedback_size)
    ...
    def _hook(self, grad):
        if self.random_projection is not None:
            return self.random_projection[:, : torch.prod(grad.shape[1:])].view(*grad.shape).to(grad.device)
        else:
            grad[:] = 0
            return grad
